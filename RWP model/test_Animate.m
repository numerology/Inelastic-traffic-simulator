function trace=test_Animate(s_mobility,s_input,time_step)
...
for timeIndex = 1:length(v_t);
    ...
    for nodeIndex = 1:s_mobility.NB_NODES
        trace(nodeIndex,timeIndex,1)=vs_node(nodeIndex).v_x(timeIndex);
        trace(nodeIndex,timeIndex,2)=vs_node(nodeIndex).v_y(timeIndex);
